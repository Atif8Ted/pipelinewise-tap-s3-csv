...
def sample_file(
    ...
    LOGGER.info("************************")
    LOGGER.info(file_handle._raw_stream)
...
def get_input_files_for_table(
    config: Dict, table_spec: Dict, modified_since: str = None
) -> Generator:
    """
    ...
    LOGGER.info(
        "Skipping files which have a LastModified value older than %s", modified_since
    )
    ...
    for s3_object in sorted(
        ...
        if (unmatched_files_count + matched_files_count) % max_files_before_log == 0:
            ...
            if (
                unmatched_files_count / (matched_files_count + unmatched_files_count)
            ) > 0.5:
                LOGGER.warning(
                    (
...
def list_files_in_bucket(
    bucket: str, search_prefix: str = None, aws_endpoint_url: Optional[str] = None
) -> Generator:
    """
